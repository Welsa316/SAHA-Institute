  ...
  <div class="min-h-screen bg-gradient-to-br from-[#001B3D] via-navy-900 to-[#001B3D] flex items-center justify-center px-6 py-12">
    ...
    <div class="relative w-full max-w-md">
      <div class="text-center mb-10">
        ...
        <p class="font-body text-[10px] tracking-[0.3em] uppercase text-white/40 font-medium mt-2">Teacher Setup</p>
      </div>
      ...
      <div class="bg-white/[0.04] backdrop-blur-sm border border-white/10 rounded-3xl p-8 md:p-10 shadow-2xl shadow-black/30">
        ...
        <div v-if="state === 'loading'" class="py-8 text-center">
          ...
          <p class="font-body text-sm text-white/50 mt-4">Checking your invite…</p>
        </div>

        <!-- Invalid / expired -->
        <div v-else-if="state === 'invalid'" class="text-center">
          <h1 class="font-heading text-2xl font-bold text-white mb-2">Link not valid</h1>
          <p class="font-body text-sm text-white/60 mb-6">This invite link is invalid or has expired. Ask the admin to send you a fresh one.</p>
        ...
        <div v-else-if="state === 'used'" class="text-center">
          <h1 class="font-heading text-2xl font-bold text-white mb-2">Already set up</h1>
          <p class="font-body text-sm text-white/60 mb-6">This account has already been set up. Just sign in with your email and password.</p>
          <RouterLink to="/admin/login" class="inline-block font-body text-sm text-academic-300 hover:text-academic-200 font-semibold">Go to sign in</RouterLink>
        </div>
        ...
        <div v-else-if="state === 'done'" class="text-center">
          ...
          <h1 class="font-heading text-2xl font-bold text-white mb-2">You’re all set</h1>
          <p class="font-body text-sm text-white/60 mb-6">Your account is ready. Sign in with <span class="text-white/80">{{ teacherEmail }}</span> and your new password.</p>
        ...
        <!-- Valid: set password -->
        <div v-else>
          <h1 class="font-heading text-2xl font-bold text-white tracking-tight mb-1">Welcome{{ teacherName ? `, ${teacherName.split(' ')[0]}` : '' }}</h1>
          <p class="font-body text-sm text-white/50 mb-8">Set a password for <span class="text-white/70">{{ teacherEmail }}</span>.</p>
