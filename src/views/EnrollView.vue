...
// The "Enroll" hub. Two paths a parent might take:
//   1. Summer Camp & STEM — manual signup over the phone. We surface a clear
//      tel: CTA + a "See full details" link to the long-form /summer-camp page.
//   2. Workshops — actual online form. Big amber CTA opens the form in an
//      animated modal so the parent never leaves the context of "what am I
//      signing up for."
//
// The old "Upcoming Event" homepage section moved here so the homepage stays
// focused on the year-round programs.
    ...
    >
      <div class="max-w-6xl mx-auto px-6 md:px-12">
        <div class="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
          ...
          >
            <div class="rounded-2xl overflow-hidden shadow-2xl shadow-navy-900/20 border border-navy-100">
              <img
          ...
          >
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-200 mb-5">
              ...
              <span class="font-body text-[10px] tracking-[0.3em] uppercase text-amber-700 font-bold">{{ t('enroll.summerBadge') }}</span>
            </div>

            <h2 class="font-heading text-3xl md:text-4xl font-extrabold text-navy-900 tracking-tight mb-4 leading-tight">
              {{ t('enroll.summerHeading1') }}<br/><span class="gradient-text">{{ t('enroll.summerHeading2') }}</span>
            </h2>

            <p class="font-body text-navy-600 mb-7 leading-relaxed">
            ...
            <div class="space-y-3 mb-8">
              <div class="flex items-center gap-3">
                <svg class="w-5 h-5 text-academic-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" />
                </svg>
                <span class="font-body text-sm text-navy-700">{{ t('enroll.summerDate') }}</span>
              </div>
              <div class="flex items-center gap-3">
                <svg class="w-5 h-5 text-academic-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="font-body text-sm text-navy-700">{{ t('enroll.summerSchedule') }}</span>
              </div>
              <div class="flex items-center gap-3">
                <svg class="w-5 h-5 text-academic-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                </svg>
                <span class="font-body text-sm text-navy-700">{{ t('enroll.summerPricing') }}</span>
              </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-4">
              ...
              <router-link
                to="/summer-camp"
                class="inline-flex items-center gap-2 font-body text-sm font-semibold text-navy-700 hover:text-navy-900 transition-colors duration-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-academic-400 focus:outline-none rounded-md"
              >
      ...
      <div class="relative max-w-5xl mx-auto px-6 md:px-12 text-center">
        ...
        >
          <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/15 border border-amber-400/30 mb-5">
            <div class="w-1.5 h-1.5 rounded-full bg-amber-400"></div>
            <span class="font-body text-[10px] tracking-[0.3em] uppercase text-amber-300 font-bold">{{ t('enroll.workshopsBadge') }}</span>
          </div>

          <h2 class="font-heading text-3xl md:text-4xl lg:text-5xl font-extrabold text-white tracking-tight mb-5 leading-tight">
            {{ t('enroll.workshopsHeading1') }} <span class="gradient-text-light">{{ t('enroll.workshopsHeading2') }}</span>
          </h2>

          <p class="font-body text-white/70 text-lg max-w-2xl mx-auto leading-relaxed mb-10">
            {{ t('enroll.workshopsDescription') }}
          </p>

          <!-- Highlights row -->
          <div class="grid sm:grid-cols-3 gap-4 md:gap-6 max-w-3xl mx-auto mb-12">
            <div class="rounded-2xl bg-white/[0.04] border border-white/10 px-5 py-5 text-left">
              <p class="font-heading text-2xl font-bold text-amber-300 mb-1">{{ t('enroll.workshopsHighlight1Value') }}</p>
              <p class="font-body text-xs uppercase tracking-wider text-white/60">{{ t('enroll.workshopsHighlight1Label') }}</p>
            </div>
            <div class="rounded-2xl bg-white/[0.04] border border-white/10 px-5 py-5 text-left">
              <p class="font-heading text-2xl font-bold text-amber-300 mb-1">{{ t('enroll.workshopsHighlight2Value') }}</p>
              <p class="font-body text-xs uppercase tracking-wider text-white/60">{{ t('enroll.workshopsHighlight2Label') }}</p>
            </div>
            <div class="rounded-2xl bg-white/[0.04] border border-white/10 px-5 py-5 text-left">
              <p class="font-heading text-2xl font-bold text-amber-300 mb-1">{{ t('enroll.workshopsHighlight3Value') }}</p>
              <p class="font-body text-xs uppercase tracking-wider text-white/60">{{ t('enroll.workshopsHighlight3Label') }}</p>
            </div>
          </div>
          ...
          <p class="mt-4 font-body text-xs text-white/50">{{ t('enroll.workshopsCtaHint') }}</p>
        </div>
      </div>
    </section>
