...
const visibleNavItems = computed(() => navItems.filter((item) => isAdmin.value || !item.adminOnly))
  ...
  <div class="min-h-screen bg-slate-50 text-navy-900">
    ...
    >
      ...
      <div class="pt-6 mt-6 border-t border-white/[0.08]">
        ...
        <p class="font-body text-sm text-white/80 truncate">{{ user?.username ?? '—' }}</p>
        <p class="font-body text-[11px] text-white/40 capitalize mb-4">{{ user?.role ?? '' }}</p>
        <button
          type="button"
          @click="handleLogout"
